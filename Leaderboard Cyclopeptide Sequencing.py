...
def linear_score(peptide: typing.Counter[int], Spectrum: typing.Counter[int]) -> int: 
...
def linear_peptide_spectrum(peptide: List[int]) -> typing.Counter[int]:  
...
def trim(Leaderboard: List[List[int]], Spectrum: typing.Counter[int], N: int) -> List[List[int]]: 
    if len(Leaderboard) == 0:
        return Leaderboard
    scores = [linear_score(linear_peptide_spectrum(peptide), Spectrum) for peptide in Leaderboard]
    ...
    while len(Leaderboard) > 0:
        ...
        for peptide in Leaderboard:
            if mass(peptide) == parent_mass(Spectrum):
                if linear_score(Counter(peptide), Spectrum) > linear_score(Counter(LeaderPeptide), Spectrum):
